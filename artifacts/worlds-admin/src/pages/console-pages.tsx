...
import { Activity, AlertTriangle, ArrowUpRight, Check, ChevronLeft, ChevronRight, Database, FileText, Flag, LockKeyhole, Plus, Search, ShieldCheck, SlidersHorizontal, UsersRound } from 'lucide-react';
...
function RevenueOperationsControls({ data }: { data: AdminData }) {
  ...
  return <div style={{ marginTop: 16 }}>
    ...
    <div className="data-table-wrap" style={{ marginTop: 12 }}><table className="data-table"><thead><tr><th>Membership plan</th><th>Cadence</th><th>Price</th><th>Catalog status</th></tr></thead><tbody>{revenue.plans.map((item) => <tr key={item.code}><td>{item.name}<div className="identity-handle mono">{item.code}</div></td><td>{item.billing_cadence}</td><td>{item.price_minor} {item.currency}</td><td><StatusBadge status={item.is_active ? 'active' : 'inactive'} /></td></tr>)}</tbody></table></div>
    <div className="data-table-wrap" style={{ marginTop: 12 }}><table className="data-table"><thead><tr><th>Credit pack</th><th>Credits</th><th>Price</th><th>Catalog status</th></tr></thead><tbody>{revenue.creditPacks.map((item) => <tr key={item.code}><td className="mono">{item.code}</td><td>{item.credits}</td><td>{item.price_minor} {item.currency}</td><td><StatusBadge status={item.is_active ? 'active' : 'inactive'} /></td></tr>)}</tbody></table></div>
    <div className="data-table-wrap" style={{ marginTop: 12 }}><table className="data-table"><thead><tr><th>Allowance plan</th><th>Allowance</th><th>Ceiling</th><th>UTC period</th></tr></thead><tbody>{revenue.allowanceConfiguration.map((item) => <tr key={`${item.planCode}:${item.allowanceKind}`}><td>{item.planCode}</td><td>{item.allowanceKind.replace(/_/g, ' ')}</td><td>{item.limit}</td><td>{item.period.replace(/_/g, ' ')}</td></tr>)}</tbody></table></div>
     <div className="data-table-wrap" style={{ marginTop: 12 }}><table className="data-table"><thead><tr><th>Recent transaction</th><th>Provider</th><th>State</th><th>Gross</th><th>Seller payable</th><th>Created</th></tr></thead><tbody>{revenue.transactions.slice(0, 8).map((item) => <tr key={String(item.id)}><td className="mono">{String(item.id)}<div className="identity-handle">{item.events.length} immutable event{item.events.length === 1 ? '' : 's'}</div></td><td>{item.provider_name ? <><span>{String(item.provider_name)}</span><div className="identity-handle mono">{String(item.provider_transaction_id ?? 'transaction pending')}</div></> : '—'}</td><td><StatusBadge status={String(item.state)} /></td><td>{String(item.gross_minor)} {String(item.currency)}</td><td>{String(item.seller_payable_minor)}</td><td>{fmtDateTime(String(item.created_at))}</td></tr>)}</tbody></table></div>
    ...
    <div className="data-table-wrap" style={{ marginTop: 12 }}><table className="data-table"><thead><tr><th>Suspicious activity</th><th>State</th><th>Events</th><th>Recorded</th><th>Review note</th></tr></thead><tbody>{revenue.suspiciousActivity.length ? revenue.suspiciousActivity.map((item) => <tr key={item.orderId}><td className="mono">{item.orderId}</td><td><StatusBadge status={item.state} /></td><td>{item.eventCount}</td><td>{fmtDateTime(item.createdAt)}</td><td>{item.reason}</td></tr>) : <tr><td colSpan={5}>No payment exceptions in the returned transaction history.</td></tr>}</tbody></table></div>
    <div className="data-table-wrap" style={{ marginTop: 12 }}><table className="data-table"><thead><tr><th>Audit time</th><th>Event</th><th>Entity</th><th>Details</th></tr></thead><tbody>{revenue.auditEvents.slice(0, 10).map((item) => <tr key={String(item.id)}><td>{fmtDateTime(String(item.created_at))}</td><td>{String(item.event_type)}</td><td>{String(item.entity_type)} · <span className="mono">{String(item.entity_id ?? '—')}</span></td><td>{JSON.stringify(item.details).slice(0, 160)}</td></tr>)}</tbody></table></div>
     <div className="data-table-wrap" style={{ marginTop: 12 }}><table className="data-table"><thead><tr><th>External event</th><th>Provider</th><th>Kind</th><th>Subject</th><th>Received</th></tr></thead><tbody>{revenue.externalEvents.slice(0, 10).map((item) => <tr key={item.id}><td className="mono">{item.providerEventId}</td><td>{item.providerName}</td><td>{item.eventKind}</td><td>{item.subjectType} · <span className="mono">{item.subjectId}</span></td><td>{fmtDateTime(item.receivedAt)}</td></tr>)}</tbody></table></div>
     <div className="notice" style={{ marginTop: 12 }}><Activity /><span>Reconciliation inbox: {revenue.reconciliation.receivedEvents} provider events received{revenue.reconciliation.latestReceivedAt ? ` · latest ${fmtDateTime(revenue.reconciliation.latestReceivedAt)}` : ''}. This is ingestion visibility only; unmatched provider events remain pending trusted reconciliation.</span></div>
