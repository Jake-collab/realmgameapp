-- Fix send_friend_request conflict inference for the partial active-request index.
--
-- friend_requests_active_pair_idx is a partial unique index, not a unique
-- constraint. PostgreSQL therefore requires the indexed columns and predicate
-- in the ON CONFLICT target.
...
BEGIN
  ...
  -- The active-pair uniqueness rule is a partial unique index, so name its
  -- columns and predicate instead of referring to it as a constraint.
  ...
  -- A concurrent sender may have won the insert. Return that request rather
  -- than emitting a notification with a null request_id.
  IF v_req_id IS NULL THEN
    SELECT id INTO v_req_id FROM friend_requests
    WHERE status='pending' AND requester_id=v_viewer AND recipient_id=v_target;
    RETURN jsonb_build_object('ok', TRUE, 'code', 'request_exists', 'request_id', v_req_id, 'state', 'outgoing_request');
  END IF;
