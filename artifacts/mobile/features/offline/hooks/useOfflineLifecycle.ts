...
import { markAllAsRead, markAsRead, updateNotificationPreferences } from '@/features/notifications/notification.service';
...
export function useOfflineLifecycle() {
  ...
  useEffect(() => {
    ...
    const removeRead = registerOfflineMutationExecutor('notification_read', async item => {
      ...
      await markAsRead(user.id, String(item.payload.notificationId), false);
      return { status: 'completed' };
    });
    const removeReadAll = registerOfflineMutationExecutor('notification_read_all', async item => {
      if (item.entityId !== user.id) return { status: 'needs_attention', errorCode: 'INVALID_PAYLOAD', message: 'This notification change needs attention.' };
      await markAllAsRead(user.id, false);
      return { status: 'completed' };
    });
    const removePreferences = registerOfflineMutationExecutor('profile_preference_save', async item => {
      await updateNotificationPreferences(user.id, item.payload as Parameters<typeof updateNotificationPreferences>[1], false);
      return { status: 'completed' };
    });
    ...
    return () => { removeRead(); removeReadAll(); removePreferences(); removeCreatorDraft(); removeProof(); removeMedia(); stopCachePersistence(); };
  }, [user?.id]);
}